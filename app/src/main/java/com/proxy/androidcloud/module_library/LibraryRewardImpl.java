...
public class LibraryRewardImpl extends AbstractUiTabHostFragmentReward {
        ...
        switch (from) {
            case TabHostRewardSelectFrom.FROM_TAB:
                if (isSelect) {
                    ...
                    mRewardHelper.set(IRewardHelper.Set.UI_EVENT, getIndex(), "通过按钮选中了工具模块");
                }
                break;
            case TabHostRewardSelectFrom.FROM_CONTENT:
                mRewardHelper.set(IRewardHelper.Set.UI_EVENT, getIndex(), "通过滑动选中了工具模块");
                break;
            case TabHostRewardSelectFrom.FROM_HELPER:
                mRewardHelper.set(IRewardHelper.Set.UI_EVENT, getIndex(), "通过设置选中了工具模块");
                break;
            default:
        }
        isSelect = true;

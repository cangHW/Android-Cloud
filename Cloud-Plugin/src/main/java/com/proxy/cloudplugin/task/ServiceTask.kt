package com.proxy.cloudplugin.task
...
abstract class ServiceTask : DefaultTask() {
    ...
    private val className = "com/proxy/service/api/plugin/DataByPlugin.class"
    private val servicePath = "com/cloud/service/\$\$Cloud\$\$Service\$\$Cache"
    ...
    fun taskAction() {
        ...
        allJars.get().forEach { jarInputFile ->
            ...
            jarFile.entries().iterator().forEach loop@{ jarEntry ->
                ...
                if (jarEntry.name.endsWith(className)) {
    ...
    private fun findService(jarEntryName: String) {
        if (jarEntryName.contains(servicePath)) {
